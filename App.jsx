...
import {
  ...
  ArrowRight, Phone, Tag, CreditCard, Banknote, Smartphone
} from "lucide-react";
...
const NOME_CONFIRMACAO = "Amigo Carlos";
...
export default function App() {
  ...
  return (
    <Shell
      ...
      view={view}
      setView={setView}
      data={{ clientes, estoque, orcamentos, ordens, financeiro, solicitacoes, servicos }}
      setters={{ setClientes, setEstoque, setOrcamentos, setOrdens, setFinanceiro, setSolicitacoes, setServicos }}
    />
  );
}
...
function ClientPortal({ onBack, onSubmit, servicos = [] }) {
  const [form, setForm] = useState({ nome: "", telefone: "", email: "", descricao: "", servicoId: "", formaPagamento: "dinheiro" });
  const [enviado, setEnviado] = useState(false);
  const [enviando, setEnviando] = useState(false);
  const [erro, setErro] = useState("");

  const escolherServico = (s) => {
    setForm({ ...form, servicoId: s.id, descricao: form.descricao ? form.descricao : s.nome });
  };
    ...
    return (
      <div className="min-h-screen flex items-start justify-center px-4 py-10 overflow-y-auto font-sans bg-[#F5EDE1]">
        <div className="max-w-sm text-center">
          ...
          <h1 className="font-serif text-2xl text-[#2B1B12] mb-2">Pedido enviado!</h1>
          ...
          {form.formaPagamento !== "dinheiro" && (
            <div className="mt-5 text-left">
              <PaymentInfoBox metodo={form.formaPagamento} />
            </div>
          )}
  ...
  return (
    <div className="min-h-screen flex items-start justify-center px-4 py-10 overflow-y-auto font-sans bg-[#F5EDE1]">
      <div className="w-full max-w-sm">
        ...
        <h1 className="font-serif text-2xl text-[#2B1B12] mb-1">Peça seu orçamento</h1>
        <p className="text-[#8A7A68] text-sm mb-6">Conte o que você precisa, a ZAINIFO entra em contato.</p>

        {servicos.length > 0 && (
          <div className="mb-6">
            <p className="text-xs font-medium text-[#6B4226] mb-2 uppercase tracking-wide">Nossos serviços</p>
            <div className="grid gap-2">
              {servicos.map((s) => (
                <button key={s.id} type="button" onClick={() => escolherServico(s)}
                  className={`text-left bg-white border rounded-md p-3 flex justify-between items-center transition-colors ${
                    form.servicoId === s.id ? "border-[#C94C36] ring-1 ring-[#C94C36]" : "border-[#E4D5C2]"
                  }`}>
                  <div>
                    <p className="text-sm font-medium text-[#2B1B12]">{s.nome}</p>
                    {s.descricao && <p className="text-xs text-[#8A7A68]">{s.descricao}</p>}
                  </div>
                  <span className="font-mono text-sm text-[#6B4226]">{brl(s.preco)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <form onSubmit={submit} className="bg-white border border-[#E4D5C2] rounded-md p-5 relative">
          <RulerDivider className="absolute -top-3 left-3 right-3" />
...
function LoginScreen({ users, setUsers, onLogin, onBack }) {
  const [mode, setMode] = useState(users.length ? "login" : "registro");
  ...
  const submit = (e) => {
    ...
    if (mode === "registro") {
      ...
      if (users.some((u) => u.usuario === usuario.trim().toLowerCase())) {
        setErro("Esse nome de usuário já existe.");
        return;
      }
      setEnviando(true);
      const novo = { id: uid(), nome: nome.trim(), usuario: usuario.trim().toLowerCase(), senha };
      setUsers([...users, novo]);
      onLogin(novo);
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-start justify-center px-4 py-10 overflow-y-auto font-sans"
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={submit} className="bg-[#F5EDE1] rounded-md p-6 shadow-xl relative">
          <RulerDivider className="absolute -top-3 left-3 right-3" />
          <div className="flex mb-5 rounded-sm overflow-hidden border border-[#D9C7B2]">
            <button type="button" onClick={() => setMode("login")}
              className={`flex-1 py-2 text-sm font-medium transition-colors ${mode === "login" ? "bg-[#2B1B12] text-[#F5EDE1]" : "bg-transparent text-[#6B4226]"}`}>
              Entrar
            </button>
            <button type="button" onClick={() => setMode("registro")}
              className={`flex-1 py-2 text-sm font-medium transition-colors ${mode === "registro" ? "bg-[#2B1B12] text-[#F5EDE1]" : "bg-transparent text-[#6B4226]"}`}>
              Criar conta
            </button>
          </div>
          ...
          <button type="submit" onClick={submit} disabled={enviando}
            ...
            {enviando ? "Entrando…" : mode === "registro" ? "Criar conta e entrar" : "Entrar"}
          </button>

          {users.length === 0 && mode === "registro" && (
            <p className="text-xs text-[#8A7A68] mt-3 text-center">
              Primeira vez aqui — essa conta será a administradora do sistema.
            </p>
          )}
        </form>
      </div>
    ...
    <div className="min-h-screen flex font-sans bg-[#F5EDE1] text-[#2B1B12]">
      ...
      <aside className="w-60 shrink-0 bg-[#2B1B12] text-[#F5EDE1] flex flex-col">
        ...
        <div className="px-5 py-4 border-t border-[#4A3020]">
          ...
          <p className="text-sm mb-3">{session.nome}</p>
...
function Dashboard({ data, setView }) {
  const { clientes, estoque, orcamentos, ordens, financeiro, solicitacoes = [] } = data;
  ...
  const cards = [
    { label: "Pedidos de clientes", value: solicitacoesNovas, icon: Inbox, onClick: () => setView("solicitacoes"),
...
function ServicosView({ servicos, setServicos }) {
  ...
  const [form, setForm] = useState({ nome: "", preco: 0, descricao: "" });
  ...
  return (
    <ViewShell title="Serviços" subtitle="Catálogo visível para os clientes no pedido à distância." onAdd={() => setShowForm(true)} addLabel="Novo serviço">
      ...
      {servicos.length === 0 ? (
        ...
        <div className="grid gap-3">
          {servicos.map((s) => (
            <div key={s.id} className="bg-white/60 border border-[#E4D5C2] rounded-md p-4 flex justify-between items-start">
              <div>
                <p className="font-medium">{s.nome}</p>
